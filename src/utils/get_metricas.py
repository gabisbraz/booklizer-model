...
def calcular_metricas(model, data, df, top_k=10, margin=1.0, device="cpu"):
    ...
    # Inicializa métricas
    precisions, recalls, f1_s, hits, average_precisions, ndcgs = [], [], [], [], [], []
    sim_pos, sim_neg = [], []
    coverage_set = set()
    diversity_list = []
    ...
    for livro in livros:
        ...
        if positivos and negativos:
            pos_id = livro2_id[random.choice(positivos)]
            neg_id = livro2_id[random.choice(negativos)]
            t_loss = criterion(
                anchor_emb,
        ...
        # Top-k recomendados
        _, topk_idx = torch.topk(cos_sim, k=top_k)
        ...
        pos_sims = [cos_sim[livro2_id[l]].item() for l in positivos] if positivos else []
        neg_sims = [
            cos_sim[i].item()
    ...
    return {
        ...
        "cosine_gap": (
            sum(sim_pos) / len(sim_pos) - sum(sim_neg) / len(sim_neg) if sim_pos and sim_neg else 0
        ),
        "triplet_loss_mean": sum(triplet_losses) / len(triplet_losses) if triplet_losses else 0,
        "coverage@k": coverage,
